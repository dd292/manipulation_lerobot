...
class S101GymEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        home_position: np.ndarray = np.zeros(6, dtype=np.float32), 
        cartesian_bounds: np.ndarray = np.asarray([[0.1, -0.1, 0], [0.28, 0.28, 0.28]]), 
    ):
        if xml_path is None:
        ...
        # Cache robot IDs
        joint_names = ["base_rotation", "pitch", "elbow", "wrist_pitch", "wrist_roll", "gripper"]
        self._s101_dof_ids = np.asarray([self._model.joint(joint_names[i]).id for i in range(0, 6)])
        self._s101_ctrl_ids = np.asarray([self._model.actuator(joint_names[i]).id for i in range(0, 6)])
    ...
    def _setup_action_space(self):
        """Setup the action space for the s101 environment."""
    ...
    def reset_robot(self):
        ...
        self._data.qpos[self._s101_dof_ids] = self._home_position
        self._data.ctrl[self._s101_ctrl_ids] = 0.0
        mujoco.mj_forward(self._model, self._data)

        # Reset mocap body to home position
        tcp_pos = self._data.sensor("pinch_pos").data
        self._data.mocap_pos[0] = tcp_pos #mocap tracking the pinch pos. 
    ...
    def get_robot_state(self):
        ...
        tcp_pos = self._data.sensor("pinch_pos").data
        # tcp_quat = self._data.sensor("pinch_quat").data
        # tcp_vel = self._data.sensor("pinch_vel").data
        # tcp_angvel = self._data.sensor("pinch_angvel").data
        qpos = self.data.qpos[self._s101_dof_ids].astype(np.float32)
        qvel = self.data.qvel[self._s101_dof_ids].astype(np.float32)
